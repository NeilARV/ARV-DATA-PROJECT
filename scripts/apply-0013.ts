// One-off applier for migration 0013 (drops email_subscription_list.msa, issue #136).
...
// Usage: tsx scripts/apply-0013.ts test|dev|both
...
async function applyTo(label: string, url: string | undefined): Promise<void> {
    ...
    const sql = neon(url);
    await sql(`ALTER TABLE "email_subscription_list" DROP COLUMN IF EXISTS "msa"`);
    const [{ exists }] = (await sql(
        `SELECT EXISTS (SELECT 1 FROM information_schema.columns
         WHERE table_name = 'email_subscription_list' AND column_name = 'msa') AS exists`,
    )) as { exists: boolean }[];
    console.log(`  ${exists ? '✗' : '✓'} ${label}: column dropped = ${!exists}`);
}

async function main(): Promise<void> {
    const target = process.argv[2] ?? 'both';
    console.log(`Applying migration 0013 to: ${target}`);
