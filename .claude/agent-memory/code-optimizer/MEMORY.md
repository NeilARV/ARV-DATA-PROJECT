...
- [Email case sensitivity](project_email_case_sensitivity.md) — users.email is case-sensitive text(); eq(users.email, x.toLowerCase()) misses mixed-case rows — prefer LOWER(TRIM()) on auth/recovery paths
