...
export const PostComposer = forwardRef<PostComposerHandle, PostComposerProps>(function PostComposer(
    { post, onSuccess, hideSubmitButton = false, onPendingChange }: PostComposerProps,
    ref,
) {
    ...
    const mutation = useMutation({
        mutationFn: async () => {
            ...
            editor.state.doc.descendants((node) => {
                if (node.type.name === 'vendorMention' && node.attrs.id)
                    vendorIds.push(String(node.attrs.id));
                if (node.type.name === 'categoryMention' && node.attrs.id)
                    categoryIds.push(Number(node.attrs.id));
            });
            ...
            if (isEditMode) {
                ...
                if (pendingFiles.length > 0) {
                    ...
                    if (failed.length > 0) {
                        throw new Error(
                            `Post created but ${failed.length} image(s) failed to upload.`,
                        );
                    }
                }
            }
        ...
        createPortal(
            ...
            >
                ...
                {dropdown.items.map((item, i) => (
                    ...
                    >
                        <span
                            className={`text-xs font-semibold ${dropdown.type === 'vendor' ? 'text-primary' : 'text-muted-foreground'}`}
                        >
    ...
    return (
        <div className="flex-shrink-0 border-t border-border bg-background pb-[env(safe-area-inset-bottom)]">
            <div className="m-3">{innerContent}</div>
            {linkPortal}
            {dropdownPortal}
        </div>
    );
