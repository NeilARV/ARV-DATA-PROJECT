  ...
  const handleSubmit = async (data: InsertCompanyContact) => {
    try {
      ...
      if (isEditing && contact) {
        const res = await apiRequest("PATCH", `/api/companies/${companyId}/contacts/${contact.id}`, payload);
        savedContact = await res.json();
        toast({ title: "Contact Updated", description: "Contact has been successfully updated." });
      } else {
        const res = await apiRequest("POST", `/api/companies/${companyId}/contacts`, payload);
        savedContact = await res.json();
        toast({ title: "Contact Added", description: "Contact has been successfully added." });
      }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input {...field} placeholder="Jane" data-testid="input-contact-firstname" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input {...field} value={field.value ?? ""} placeholder="Acquisitions Manager" data-testid="input-contact-title" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
