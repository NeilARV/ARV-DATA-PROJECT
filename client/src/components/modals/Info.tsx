...
export default function InfoContent({ onClose }: InfoContentProps) {
  ...
  const phone = rm.phone
    ? rm.phone.includes("(") ? rm.phone : formatPhoneNumber(rm.phone)
    : null;
      ...
      <div className="flex items-center gap-4 py-4">
        ...
        <div className="flex flex-col items-start gap-1.5 min-w-0">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <a href={`mailto:${rm.email}`} className="text-primary hover:underline truncate">
              {rm.email}
            </a>
          </div>
          {phone && (
            <div className="flex items-center gap-2 text-sm">
              ...
              <a href={`tel:${rm.phone?.replace(/\D/g, "")}`} className="text-foreground hover:underline">
                {phone}
              </a>
            </div>
          )}
