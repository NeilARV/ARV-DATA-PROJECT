...
// arms-length transaction and decides notifiability from company_members. Mock only the two DB-backed
// boundaries it calls — the transaction read and the member lookup — and let the REAL ownership logic
...
describe('resolveOwner', () => {
    it('resolveOwner — arms-length buyer is a company with members — notifiable', async () => {
    ...
    it('resolveOwner — company owner with no members — not notifiable, company id retained', async () => {
        txns.getPropertyTransactions.mockResolvedValue([
            armsLength({ buyerId: COMPANY_ID, buyerName: 'ACME LLC' }),
        ]);
    ...
    it('resolveOwner — individual owner (buyerName only, no buyerId) — not notifiable, never queries members', async () => {
        txns.getPropertyTransactions.mockResolvedValue([
