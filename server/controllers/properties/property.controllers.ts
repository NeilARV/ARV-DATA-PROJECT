...
export async function removeProperty(req: Request, res: Response) {
    try {
        ...
        console.log(`[DELETE] Successfully deleted property: ${id} (SFR Property ID: ${deleted.sfrPropertyId})`);
        return res.json({ message: "Property deleted successfully", id: deleted.id, sfrPropertyId: deleted.sfrPropertyId });
    } catch (error) {
...
export async function postProperty(req: Request, res: Response) {
    try {
        ...
        switch (result.status) {
            case "missing-config":
                return res.status(500).json({ message: "SFR API not configured", error: "SFR_API_KEY and SFR_API_URL must be set" });
            case "sfr-error":
                return res.status(result.httpStatus).json({ message: "Failed to fetch property from SFR API", error: result.error });
            case "not-found":
                return res.status(404).json({ message: "Property not found in SFR API", error: "No property data returned" });
            case "updated":
                return res.json({ message: "Property updated successfully", id: result.id, sfrPropertyId: result.sfrPropertyId });
            case "created":
                return res.json({ message: "Property created successfully", id: result.id, sfrPropertyId: result.sfrPropertyId });
        }
    } catch (error) {
