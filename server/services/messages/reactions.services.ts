...
// readable (public, non-archived) channel. Returns the channel id for the broadcast.
async function getReactableChannelId(messageId: string): Promise<string> {
    const [row] = await db
        .select({
...
export async function addReaction(
    ...
    const channelId = await getReactableChannelId(messageId);
