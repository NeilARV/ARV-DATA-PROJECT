...
function mapPropertyRow(item: PropertyWithStatus, msa: string): typeof properties.$inferInsert {
...
export async function insertProperties(
    ...
    for (const item of items) {
        ...
        const itemStatuses: string[] = Array.isArray(itemAny.statuses)
            ? (itemAny.statuses as string[])
            : [
                  ((item.property as Record<string, unknown>).status as string | undefined) ??
                      'in-renovation',
              ];
        ...
        if (structureRow) {
            ...
            await db.insert(structures).values(structureRow).onConflictDoUpdate({
        ...
        if (exemptionRow) {
            ...
            await db.insert(exemptions).values(exemptionRow).onConflictDoUpdate({
        ...
        if (parcelRow) {
            ...
            await db.insert(parcels).values(parcelRow).onConflictDoUpdate({
        ...
        if (schoolRow) {
            ...
            await db.insert(schoolDistricts).values(schoolRow).onConflictDoUpdate({
        ...
        if (preForeclosureRow) {
            ...
            await db.insert(preForeclosures).values(preForeclosureRow).onConflictDoUpdate({
        ...
        if (lastSaleRow) {
            ...
            await db.insert(lastSales).values(lastSaleRow).onConflictDoUpdate({
        ...
        if (currentSaleRow) {
            ...
            await db.insert(currentSales).values(currentSaleRow).onConflictDoUpdate({
        ...
        const mappedTxRows = transactions
            ...
            .filter(
                (row): row is NonNullable<ReturnType<typeof mapTransactionRow>> => row !== null,
            );
        ...
        const merged: MergeEntry[] = [
            ...
            ...userCreatedRows.map((r) => ({
                kind: 'user' as const,
                id: r.id,
                recordingDate:
                    typeof r.recordingDate === 'string'
                        ? r.recordingDate
                        : ((r.recordingDate as Date | null)?.toISOString().split('T')[0] ?? ''),
            })),
        ].sort((a, b) => {
