...
/** Counters returned by the compute routine — the step and backfill both report them. */
 ...
 */
export async function computeSupplementalTaxForProperties(
    propertyIds: string[],
    ...
    for (const propertyId of ids) {
        ...
        result.caProperties += 1;

        try {
            const sorted = sortTransactionsDesc(
                expandTruncatedNames(txsByProperty.get(propertyId) ?? []),
            );
            // Newest-first once per property; each slot takes the first row at/before
            // its lien year.
            const propertyAssessments = (assessmentsByProperty.get(propertyId) ?? [])
            ...
            for (let i = 0; i < sorted.length; i++) {
                ...
                const schedule = getSupplementalTaxSchedule(address.state, saleTx.saleDate);
                if (schedule.length === 0) {
                    result.skippedZeroOrInvalid += 1; // unparseable sale date
                    continue;
                }
                ...
                for (const slot of schedule) {
                    ...
                    const rollValue = propertyAssessments.find(
                        (a) => a.assessedYear <= slot.fiscalYear,
                    );
                ...
                if (rowsForTx === 0) {
                    if (missingPriorValue) result.skippedNoPriorValue += 1;
                    else result.skippedZeroOrInvalid += 1;
                }
            }
        } catch (err) {
 ...
 * reads everything back from the DB via the shared compute routine. The consumer only
...
export async function insertSupplementalTaxBills(
    ...
    const result = await computeSupplementalTaxForProperties(propertyRows.map((r) => r.id));

    console.log(
        `[${cityCode}] Supplemental tax: ${result.billRowsWritten} bill(s), ` +
            `${result.refundRowsWritten} refund(s) written across ${result.caProperties} properties; ` +
