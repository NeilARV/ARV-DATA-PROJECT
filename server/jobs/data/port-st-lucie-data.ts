...
const PORT_ST_LUCIE_MSA = "Port St. Lucie, FL";
const CITY_CODE = "PSL";

export async function syncPortStLucieData() {
    console.log(`[${CITY_CODE} SYNC] Syncing Data for MSA: ${PORT_ST_LUCIE_MSA}`);
    ...
    try {
        let lastSaleDate: string | null = await fetchLastSaleDate(PORT_ST_LUCIE_MSA);
        
        if (lastSaleDate == null) {
            throw new Error(`[${CITY_CODE} SYNC] Cannot get last sale date for MSA: ${PORT_ST_LUCIE_MSA}; sync aborted.`);
        }
        ...
        if (lastSuccessfulSaleDate !== null) {
            ...
            await updateLastSaleDate(PORT_ST_LUCIE_MSA, CITY_CODE, storedDate);
        }

        console.log(
            `[${CITY_CODE} SYNC] Complete Syncing Data for MSA: ${PORT_ST_LUCIE_MSA} ` +
        ...
        console.error(`[${CITY_CODE} SYNC] Fatal error syncing ${PORT_ST_LUCIE_MSA}:`, errorMessage);
        throw error;
    }
}