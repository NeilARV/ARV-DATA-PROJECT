...
export class NeonSessionStore extends Store {
    ...
    async set(sid: string, session: any, callback?: (err?: any) => void): Promise<void> {
        try {
            ...
            await db.insert(sessions).values({ sid, sess, expire }).onConflictDoUpdate({
                target: sessions.sid,
                set: { sess, expire },
            });

            console.log(
                `[SessionStore] Session saved: ${sid}, expires: ${new Date(expire * 1000).toISOString()}`,
            );
    ...
    async cleanup(): Promise<void> {
        try {
            const now = Math.floor(Date.now() / 1000);
            const deleted = await db.delete(sessions).where(lt(sessions.expire, now)).returning();
