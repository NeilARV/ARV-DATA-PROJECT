...
/**
 * WHERE condition matching the county filter against properties.county / addresses.county.
...
export function countyScopeCondition(
    ...
    return (
        or(
            sql`LOWER(TRIM(${properties.county})) IN (${list})`,
            sql`LOWER(TRIM(${addresses.county})) IN (${list})`,
        ) ?? null
    );
}
