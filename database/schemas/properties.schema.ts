...
export const properties = pgTable(
    ...
    (t) => [index('idx_properties_county_lower').on(sql`lower(trim(${t.county}))`)],
);
...
export const valuations = pgTable(
    'valuations',
    {
        ...
        forecastStandardDeviation: decimal('forecast_standard_deviation', {
            precision: 18,
            scale: 15,
        }),
...
export const propertyTransactions = pgTable(
    ...
    (t) => [
        ...
        index('idx_pt_buyer_sort1')
            .on(t.buyerId)
            .where(sql`${t.sortOrder} = 1`),
