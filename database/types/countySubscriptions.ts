import { z } from 'zod';
import { userCountySubscriptions, emailSubscriptionListCounties } from '../schemas/msas.schema';
import { insertUserCountySubscriptionSchema } from '../inserts/countySubscriptions.insert';

export type UserCountySubscription = typeof userCountySubscriptions.$inferSelect;
export type EmailSubscriptionListCounty = typeof emailSubscriptionListCounties.$inferSelect;
export type InsertUserCountySubscription = z.infer<typeof insertUserCountySubscriptionSchema>;
export type {
    CountySubscriptionInput,
    CountySubscriptionSelection,
} from '../validation/countySubscriptions.validation';
